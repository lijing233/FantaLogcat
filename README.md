...
- 在系统开放 ADB Clipboard shell 接口的设备上读取手机剪贴板；受 Android/OEM 权限限制时会明确说明不可用，不会误显示内容。
...
- 支持 USB、Android 11+ 无线配对和 USB 一键转无线；保存最近连接，可自动恢复 TCP/IP 无线设备。
...
公开发布版本仍需由维护者使用 Apple Developer ID 签名并完成公证。未公证的构建可能被 Gatekeeper 拦截；请先在 Finder 中右键应用选择“打开”。如果系统仍然阻止，请手动进入“系统设置 → 隐私与安全性”，在对应页面为 FantaLogcat 选择“仍要打开”，不要全局关闭 Gatekeeper。成功安装 1.0.10 或更高版本后，后续版本可通过应用内 Sparkle 更新；开启“自动检查更新”时，应用每次启动都会静默检查一次。
...
- Android 11+：在设备“开发者选项 → 无线调试”中选择扫码或配对码；在 FantaLogcat 的“无线调试”页完成配对。
- USB 转无线：先以 USB 调试连接，在设备菜单中选择“USB 转无线”。应用会探测 Wi‑Fi 地址、执行 `adb tcpip`、连接并切换到无线会话。
- 已保存的 TCP/IP 设备：下次启动时会尝试恢复最近连接；可在连接页关闭“自动恢复”或删除该记录。
...
- Android 设备厂商对“读取手机剪贴板”的 ADB 接口支持并不一致；若系统未开放 Clipboard shell 接口，FantaLogcat 会提示不可用，仍可使用“粘贴”把 Mac 剪贴板内容发送到手机输入框。
